...
def SL宝石(gf: GemFilter, saveIndex=1, 背包上界=5, target=GemLevel.顶级, mode="bug"):
    """
    ...
    for key in ["融合器-宝石属性文本", "融合器-宝石评级文本"]:
        if key not in r:
    ...
    required_points = [
        # mode == bug
        "融合器-确认融合", "融合器-确认融合-是", "融合器-融合成功-确认", "融合器-材料详情-关闭", "背包格00",
    ...
    # 卡背包 bug 重置宝石：https://www.bilibili.com/video/BV1x8411j7Wx?t=179
    ...
    if mode == "bug":
        ...
        while True:
            ...
            # 出货了就保存, 退出死循环
            if isGood:
                toast(f"第 {cnt} 次: {g} | 已出货, 请手动保存")
                break
            else:
                pClick(v["融合器-材料详情-关闭"], before=min)
        return
    else:
        toast(f"本版本仅支持 mode = bug, 不支持 mode = {mode}")
        return
