...
def _new_刺客():
    ...
    gf = GemFilter([
        ...
        # Entry.from_str("MP增加: 70"),
        # Entry.from_str("MP恢复: 4"),
        Entry.from_str("暴击率: 5%"),
        Entry.from_str("暴击伤害增加率: 9.5%"),
        Entry.from_str("回避率: 3%"),
        Entry.from_str("MP恢复: 4"),
        # Entry.from_str("HP吸收: 8%"),
        # Entry.from_str("魔法抵抗率: 4%"),
        # Entry.from_str("暴击伤害抵抗率: 22%"),
    ])
    SL孔位(saveIndex=2, packIndex=1, posID="00", is可强化装备=False)
    SL孔位(saveIndex=2, packIndex=1, posID="01", is可强化装备=True)
    SL孔位(saveIndex=2, packIndex=1, posID="02", is可强化装备=True)
    SL宝石(gf, saveIndex=2, 背包上界=3, target=GemLevel.高级, mode="FULL")
