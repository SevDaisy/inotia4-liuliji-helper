from .constrants import toast, pClick, pSlide, imgFind

from .db import loadPoint
...
def SL强化(saveIndex=1, packIndex=1, maxRetry=15, maxSuccess=15):
    ...
    for item in [
        ...
        "菜单", "背包菜单", f"背包页{packIndex}", "背包分格21", "背包分格22",
            "强化成功", "主菜单页", "保存选项", "确认保存"]:
    ...
    while maxRetry <= 0 or cnt < maxRetry:
        while success < maxSuccess:
            pSlide(v["背包分格21"], v["背包分格22"], before=short)
    ...
    pClick(v["背包分格22"], before=short, msg="打开背包")


SL强化(saveIndex=1, packIndex=1, maxRetry=0, maxSuccess=12)
